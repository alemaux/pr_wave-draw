...
InputPoint::InputPoint(unsigned int nb_samples, FLOAT time_step) {
  pos = VEC2(0, 0);
...
void InputPoint::computeSpectrum() {
  ...
  // for (unsigned int i = 0; i < nb_frequencies; ++i) {
...
COMPLEX InputPoint::amplitude(FLOAT frequency) const {
  unsigned int index = frequency/frequency_step;
...
void InputPoint::plotSpectrogram() const {
  ...
  for (it = spectrogram.begin(); it != spectrogram.end(); ++it, ++i) {
    for (unsigned int j = 0; j < nb_frequencies; ++j) {
